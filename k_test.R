...
start.dt <- lubridate::as_datetime(which(extract(d$ts, cbind(spars$x, spars$y-25), method = "bilinear") - 273
                                         >= 8)[1] * 86400,
...
coa1 <- data.frame(x = runif(1,1200,1800), y = rep(2450,1))
  ...
  simKcam(
      ...
      mpar = with(subset(spars, river == "Campbellton"), 
                  sim_par(growth=FALSE,
                          scenario = "as",
                          shelf = TRUE, ## but only for land avoidance (set in moveKcam.R)
                          N = (440 - om.delay) * 24,
                          start = cbind(x-10,y-40), 
                          start.dt = start.dt,
                          mdir = NULL,
                          coa = cbind(c(coa1$x[i],x-10), c(coa1$y[i],y-40)), #cbind(c(1550,x-10),c(775,y-40)), 
