crwr <- function(rstr, theta=c(0, 0.9), s=c(3,5), initPos=c(NA,NA), 
  ...
  if(all(!is.na(initPos))) {
    init <- initPos
    if(raster::extract(rstr,cbind(init[1],init[2]), method ="simple") == 0) {
  ...
  for(i in 1:nwin){
    ...
    path.fwd[rows.i,] <- bcrw(theta=theta, s=s, initPos=init,
