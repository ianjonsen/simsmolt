...
#' @export
#' 
temp_brw <- function(n = 1, i = NULL, mpar = NULL, d1 = NULL, data, xy = NULL, ts = NULL, ts.rng = NULL, b) {
  
  if (mpar$a > 0)
    st <- rweibull(n, mpar$a, b)
  else if(mpar$a == 0 & b > 0) {
    st <- b
  } else {
    st <- 0
  }
  ...
  if(all(xy[1] >= 950,
    ...
    if(d2l <= mpar$buffer & d2l > 2) {
      mu <- (extract(data$land_dir, rbind(xy))  + 0.5 * pi) %% (pi)
      rho <- mpar$rho
    } else if(d2l <= 2) {
      mu <- (extract(data$land_dir, rbind(xy)) + pi) %% (pi) ## move in opposite direction of land if within 2km
      rho <- 0.95
    } else if(d2l > mpar$buffer) {
        mu <- mpar$mdir /180 * pi
        rho <- mpar$rho
      } 
    } else {
  ...
  if(new.d2l == 0) {
    ## find all nearby cells within 4 km & select the one farthest from land
    cells <- extract(data$land, rbind(new.xy), buffer = 3, cellnumbers = TRUE, df = TRUE)
    if(max(cells[,3], na.rm = TRUE) == 0) {
      ## if still on land then redo but increase buffer size
