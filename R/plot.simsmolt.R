...
b700 <- rasterToPoints(s$data$b700.dist) %>% data.frame()
names(land)[3] <- "d"
names(b700)[3] <- "z"


if(is.null(xlim)) xlim <- extendrange(s$sim$x, f = 1)
if(is.null(ylim)) ylim <- extendrange(s$sim$y, f = 0.25)

coa <- data.frame(x = s$data$coa[1], y = s$data$coa[2])

m <- ggplot() + 
  coord_fixed(ratio = 1.84, xlim = xlim, ylim = ylim, expand = TRUE) + 
...
#  scale_fill_gradient2(low = "#053061", mid = "#43a2ca", high = "#e0f3db", guide = "none") +
  ...
  theme_minimal() + 
  geom_point(data = coa, aes(x, y), colour = "red", size = 1) +
  geom_point(data = s$sim, aes(x, y), colour = "firebrick", size = 0.01) +
  ...
  geom_contour(data = b700, aes(x,y,z=z), breaks=1, col="white", lwd=0.5)
   if(!is.null(s$detect)) {
